...
/// Returns the directory where saved channel profiles are stored.
fn channels_dir() -> PathBuf {
    let base = dirs::config_dir().expect("cannot determine config directory");
    base.join("dispatch").join("channels")
}

/// Sanitise a channel name to a safe filename (lowercase, alphanumeric + hyphens).
...
/// Save the current config.toml as a named channel.
pub fn channel_save(name: &str) {
    let src = config_path();
    if !src.exists() {
        eprintln!("No config.toml found. Run `dispatch` once to generate defaults.");
        std::process::exit(1);
    }
    let dir = channels_dir();
    fs::create_dir_all(&dir).expect("failed to create channels directory");
    let safe = sanitize_channel_name(name);
    let dest = dir.join(format!("{safe}.toml"));
    fs::copy(&src, &dest).expect("failed to save channel");
    println!("Channel saved: {safe}");
    println!("  {}", dest.display());
}

/// Load a named channel, overwriting the active config.toml.
pub fn channel_load(name: &str) {
    let safe = sanitize_channel_name(name);
    let src = channels_dir().join(format!("{safe}.toml"));
    if !src.exists() {
        eprintln!("Channel not found: {safe}");
        eprintln!("Run `dispatch channel list` to see saved channels.");
        std::process::exit(1);
    }
    // Validate the channel file parses correctly before overwriting.
    let raw = fs::read_to_string(&src).expect("failed to read channel file");
    let _cfg: Config = toml::from_str(&raw).unwrap_or_else(|e| {
        eprintln!("Channel file is invalid: {e}");
        std::process::exit(1);
    });
    let dest = config_path();
    fs::copy(&src, &dest).expect("failed to load channel");
    println!("Channel loaded: {safe}");
    println!("  config.toml updated -- restart the console to apply.");
}
...
pub fn channel_list() {
    let dir = channels_dir();
    if !dir.exists() {
        println!("No saved channels.");
        println!("Use `dispatch channel save <name>` to save the current config.");
        return;
    }
    let mut entries: Vec<_> = fs::read_dir(&dir)
        .expect("failed to read channels directory")
        .filter_map(|e| e.ok())
    ...
    for entry in &entries {
        ...
        // Try to parse for a summary line.
        if let Ok(raw) = fs::read_to_string(&path) {
            if let Ok(cfg) = toml::from_str::<Config>(&raw) {
                println!(
                    "  {:<20} port={:<6} user={:<12} agents={}",
                    name,
                    cfg.server.port,
                    cfg.identity.user_callsign,
                    cfg.callsigns().len(),
                );
                continue;
            }
...
pub fn channel_delete(name: &str) {
    let safe = sanitize_channel_name(name);
    let path = channels_dir().join(format!("{safe}.toml"));
    if !path.exists() {
        eprintln!("Channel not found: {safe}");
        std::process::exit(1);
    }
    fs::remove_file(&path).expect("failed to delete channel");
    println!("Channel deleted: {safe}");
}

/// Print the contents of a saved channel.
pub fn channel_show(name: &str) {
    let safe = sanitize_channel_name(name);
    let path = channels_dir().join(format!("{safe}.toml"));
    if !path.exists() {
        eprintln!("Channel not found: {safe}");
    ...
    let raw = fs::read_to_string(&path).expect("failed to read channel file");
    println!("--- channel: {safe} ---");
    println!();
    print!("{raw}");
}
